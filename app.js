...
const CPU_MODEL="onnx-community/SmolLM-135M-Instruct-ONNX";
const CPU_DTYPE="q4";
...
async function loadCpuModel(){
  ...
  status.textContent="GPU unavailable. Starting verified CPU/WASM model…";
  $("progress").value=0;
  try{
    ...
    mode="cpu";
    $("progress").value=1;
    status.textContent="Local CPU brain ready.";
    add("AEGIS CPU brain is ready. Inference runs locally in this browser via WebAssembly.");
  }catch(e){
    mode=null;
    status.textContent=`CPU model failed: ${e?.message||e}`;
...
async function loadModel(){
  ...
  try{
    if(navigator.gpu){
      ...
      if(adapter){
        status.textContent="GPU available. Loading local GPU brain…";
        ...
        mode="gpu";
        status.textContent="Local GPU brain ready.";
        $("progress").value=1;
    ...
    add(`GPU brain unavailable. ${e?.message||e}. Switching to CPU/WASM…`);
    await loadCpuModel();
...
async function send(){
  ...
  try{
    if(mode==="gpu"){
      ...
      const r=await cpuGenerator(messages,{max_new_tokens:128,do_sample:false});
      const generated=r?.[0]?.generated_text;
      let answer=Array.isArray(generated)?generated.at(-1)?.content:generated;
      if(typeof answer!=="string")answer="No response.";
      add(answer.trim()||"No response.");
    }
