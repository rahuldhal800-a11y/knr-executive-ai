...
A model is not considered an AEGIS release merely because training completes. A release candidate must pass general instruction following, tool-call correctness, coding tests, factuality checks, cybersecurity defensive/authorized-use tests, refusal and permission-boundary tests, and regression tests against the previous release.
